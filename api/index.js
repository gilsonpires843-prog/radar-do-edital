...
<head>
    ...
    <title>LicitaHub Suite - Painel de Controle</title>
    <style>
        :root { 
        ...
        /* Sidebar no padrão do LicitaHub Suite */
        ...
        .brand-logo { background: var(--primary-color); width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: bold; font-size: 1.2rem; }
        .brand-text h2 { margin: 0; font-size: 1rem; color: #fff; }
        .brand-text span { font-size: 0.7rem; color: var(--text-muted); text-transform: uppercase; }
        
        .sidebar-menu { list-style: none; padding: 0; margin: 0; overflow-y: auto; flex: 1; }
        .sidebar-menu li { padding: 10px 14px; border-radius: 8px; cursor: pointer; margin-bottom: 4px; font-size: 0.9rem; display: flex; align-items: center; gap: 12px; color: var(--text-muted); transition: all 0.2s; }
        .sidebar-menu li:hover, .sidebar-menu li.active { background-color: #1e293b; color: #fff; }
        .sidebar-menu li.active { border-left: 4px solid var(--primary-color); padding-left: 10px; }
        ...
        /* Módulo de Cores da Effecti */
        .status-box { border-radius: 8px; padding: 25px; text-align: center; color: white; font-weight: bold; transition: background-color 0.4s ease; margin-top: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); }
...
<div class="sidebar">
    ...
    <ul class="sidebar-menu">
        ...
        <li class="active">🎯 Radar de Licitações</li>
        ...
        <li>💬 Monitoramento de Chat</li>
    </ul>
</div>

<div class="main-content">
    <h2>🎯 Radar de Licitações</h2>
    <p style="color: var(--text-muted); margin-bottom: 25px;">Monitore editais públicos de qualquer produto e gerencie o Robô de Lances integrado.</p>

    <div class="dashboard-grid">
        <div class="card">
            <h3>Filtro Avançado de Alvos</h3>
            <div class="search-group">
                <input type="text" id="inputBusca" onkeyup="verificarDigitacao(event)" placeholder="Digite qualquer produto para buscar (ex: notebook, medicamentos)...">
                <button onclick="executarBuscaUniversal()">Buscar Portais</button>
            </div>
            
            <div id="painelSubfiltros" class="subfiltro-container">
                <div class="subfiltro-title">🎯 Palavras-Chave Secundárias de Filtro Automático:</div>
                <div class="search-group" style="margin-bottom: 5px;">
                    <input type="text" id="inputSubTermo" placeholder="Adicione restrições específicas (ex: marcas, modelos, descartável)...">
            ...
            <div id="containerResultados" class="results-box">Insira uma palavra-chave universal acima para iniciar o rastreamento estratégico do robô.</div>
        </div>
        
        <div class="card">
            <h3>Módulo Robô de Lances (Estilo Effecti)</h3>
            ...
            <div id="configAtiva" style="font-size: 0.85rem; margin-bottom: 15px; color: #38bdf8; font-weight: 600;">Aguardando termo de pesquisa...</div>
            
            <div id="painelRobo" class="status-box winning">
                <div style="font-size: 0.85rem; text-transform: uppercase; opacity: 0.8; letter-spacing: 0.5px;">Melhor Lance Atual</div>
...
<script>
    ...
    function verificarDigitacao(event) {
        ...
        if (termosCorretos[termoPrincipal.toLowerCase()]) {
            ...
            container.innerHTML = `<div style="color:#ef4444; padding: 10px;">⚠️ Erro ortográfico detectado! Você quis dizer: <button onclick="aplicarCorrecao('${sug}')" style="background:#10b981; color:white; border:none; border-radius:4px; padding:4px 8px; cursor:pointer; font-weight:bold;">${sug}</button>?</div>`;
        ...
        if (termoPrincipal.length > 2) {
            ...
            document.getElementById('configAtiva').innerText = `🎯 Alvo Ativo do Robô: ${termoPrincipal.toUpperCase()}`;
        } else {
            painel.style.display = "none";
        }
